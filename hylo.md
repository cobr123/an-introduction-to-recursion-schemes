...
И теперь, когда мы это сделали, я хотел бы, чтобы вы рассказали мне о различиях между `hylo` и `cata`. [Вот](./cata.md#Именование) реализация `cata`, к которой мы пришли, если вам нужно освежить память.
